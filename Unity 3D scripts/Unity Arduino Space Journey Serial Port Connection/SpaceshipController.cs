...
public sealed class SpaceshipController : MonoBehaviour
{
    ...
    public float x_center_float = 0f; // command to move joystick
    public int x_center_int = 0; // command to move joystick
	...
	private void Awake()
	{
		...
		// DEV PER SECOND
        port1 = new SerialPort("COM7", 9600);                 
        port1.Open();
        port1.WriteLine("on 1");
		// DEV PER SECOND
	...
	private void UpdateOrientationAndPosition()
	{
        ...
        // DEV PER SECOND
        x_center_float = SmoothedInput.y * 40f;
        x_center_int = 50 +(int)x_center_float;
        port1.WriteLine("x_center_100_perc " +x_center_int);
		// DEV PER SECOND
